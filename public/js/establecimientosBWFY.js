...
const newCompanyTitle = document.getElementById('new-company-title')
const newCompanyDescription = document.getElementById('new-company-description')
const newCompanyPhoneNumber = document.getElementById('new-company-phone-number')
const newCompanyDirection = document.getElementById('new-company-address')
...
const modificationEstablishmentSubmit = document.querySelector('.modification-establishment-submit')
...
function setImage(image){
  ...
  companyLogo.src = image
      ...
      establishmentId = []
        db.collection(collectionPath).doc(category).collection('establishments').get().then(query =>{
            ...
            query.forEach(element =>{
                ...
                description
                establishmentId.push(doc.establishment_id)
                fillEstablishment(doc.establishment, description, doc.phone_number, doc.direction)
                companyNames = document.querySelectorAll('.company-name');
                companyDescription = document.querySelectorAll('.company-description')
                companyPhoneNumber = document.querySelectorAll('.company-phone-number')
                companyDirection = document.querySelectorAll('.company-direction')
                ...
                actionEdit.forEach((element, index) => {
                    element.addEventListener('click', e => {  
                        establishmentId = establishmentId[index]  
                        newCompanyTitle.value = companyNames[index].textContent
                        newCompanyDescription.value = companyDescription[index].textContent
                        newCompanyPhoneNumber.value = companyPhoneNumber[index].textContent
                        newCompanyDirection.value = companyDirection[index].textContent
                ...
                actionsDelete.forEach((actionDelete, index) =>{
                  actionDelete.addEventListener('change', e => {
                    if(actionDelete.defaultChecked) {
                      lockEstablishment(establishmentId[index]) 
                      lockPromos(establishmentId[index])
                    } else {
                      unlockEstablishment(establishmentId[index]) 
                      unlockPromos(establishmentId[index])
                    }
                  })
                });
                i++
    ...
    function fillEstablishment(name, description, phoneNumber, direction){
      establishmentList.innerHTML += `<div class="col l12 m6 s6" style="padding-left: 50px; padding-right: 50px;">
      <div class="row card-panel">
        <div class="col s12" style="border-bottom: black 1px solid; padding: 15px;">
            <p style="color: black; font-weight: bold; font-size: 20px;" class="company-name">${name}</p>
            <p style="color: ${setColor}" class="company-description">${description}</p>
            <p style="color: gray" class="company-phone-number">${phoneNumber}</p>
            <p style="color: gray" class="company-direction">${direction}</p>
        </div>
    ...
    function addEstablishmentFun(){
        ...
        new compressor(globalEstablishment,{
            quality: 0.6,
    ...
    console.log(promoId)
    var promRef = db.collection(collectionPath).doc(elems.value).collection('establishments').doc(establishment.value)
    ...
    function lockEstablishment(id){
      console.log(id)
        var promRef = db.collection(collectionPath).doc(elems.value).collection('establishments').doc(id)
        return promRef.update({
            estado: false
        })
        .then(e => {
          console.log('El establecimiento fue desactivado exitosamente')
          getEstablisments(elems.value)
          alert('El establecimiento fue desactivado exitosamente')
        })
        .catch(error => {
      ...
      function lockPromos(id){
        db.collection(collectionPath).doc(elems.value).collection('establishments').doc(id).collection(subCollectionPath).get().then(query =>{
          query.forEach(docData => {
            var promRef = db.collection(collectionPath).doc(elems.value).collection('establishments').doc(id).collection(subCollectionPath).doc(docData.data().promotion_id)
          return promRef.update({
            estado: false
        })
          })
    ...
    function unlockEstablishment(id){
      ...
      .then(e => {
        console.log('El establecimiento fue activado exitosamente')
        getEstablisments(elems.value)
        alert('El establecimiento fue activado exitosamente')
      })
      .catch(error => {
        getEstablisments(elems.value)
        alert('something went wrong: ' + error)
    ...
    modificationEstablishmentSubmit.addEventListener('change', e => {
      const reader = new FileReader();
      const file = e.target.files[0]
      globalEstablishment = file
      reader.readAsDataURL(file);
  ...
  modifyButton.addEventListener('click', e => {
    if(modificationFilePath == ''){
        loader.style.display = 'block'
        loaderBackground.style.display = 'block' 
        var newPhoneNumber = document.getElementById('new-company-phone-number')
        var newAddress = document.getElementById('new-company-address')
        var newTitle = document.getElementById('new-company-title')
        var newDescription = document.getElementById('new-company-description')
        console.log(globalEstablishment)
        console.log(elems.value)
        console.log(establishment.value)
        console.log(promoId)
        const ref = storage.child('alliance-image/promotions/' + globalEstablishment.name)
        ref.put(globalEstablishment).then((snap) => {
            console.log("uploaded success");
            snap.ref.getDownloadURL().then(imageUrl =>{
            console.log(imageUrl)
            var promRef = db.collection(collectionPath).doc(elems.value).collection('establishments').doc(establishmentId)
            return promRef.update({
                description: newDescription.value,
                establishment: newTitle.value,
                direction: newAddress.value,
                phone_number: newPhoneNumber.value,
                establishment_image:imageUrl
            })
            .then(e =>{
